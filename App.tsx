...
export default function App() {
  ...
  if (mode === 'website') {
    return <LandingPage onEnterApp={() => setMode('platform')} onGoToSupport={() => { setMode('platform'); setActiveView('support'); }} />;
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex overflow-hidden">
        ...
        <div className="flex flex-col h-full">
          <div className="p-8 flex items-center justify-between">
            <div className="flex items-center space-x-3 cursor-pointer" onClick={() => setMode('website')}>
              ...
              <h1 className="text-xl font-black text-slate-800">HydroGrow</h1>
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="h-20 glass-effect flex items-center justify-between px-10 border-b border-slate-100 shrink-0">
           ...
           <div onClick={() => setActiveView('settings')} className="w-10 h-10 rounded-full bg-slate-200 cursor-pointer overflow-hidden"><img src="https://picsum.photos/seed/user/100" alt="User" /></div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-10 scroll-smooth">
          ...
          {activeView === 'support' && (
            <div className="max-w-4xl mx-auto space-y-12">
               <h2 className="text-4xl font-black text-center">Support Us</h2>
               <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                  {[5, 15, 50].map(amt => (
                    <Card key={amt} title={`$${amt} Donation`}>
          ...
          {activeView === 'settings' && (
            <div className="max-w-4xl mx-auto space-y-8">
              <Card title="Developer Settings">
                 <div className="space-y-4">
                   <p className="text-sm text-slate-500">Default PayPal ID: <strong>{paypalId}</strong></p>
          ...
          {/* Other views omitted for brevity, adding back logic for them */}
          {activeView === 'setups' && (
             <div className="space-y-8">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 {setups.map(s => (
                   <Card key={s.id} title={s.name} action={<button onClick={() => deleteSetup(s.id)} className="text-red-500"><Trash2 size={16}/></button>}>
          ...
          {activeView === 'plants' && (
             <div className="space-y-8">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 {plants.map(p => (
                   <Card key={p.id} title={p.name} action={<button onClick={() => deletePlant(p.id)} className="text-red-500"><Trash2 size={16}/></button>}>
          ...
          {activeView === 'inventory' && (
            <div className="space-y-8">
               <div className="flex justify-between"><h3 className="text-2xl font-bold">Inventory</h3><Button onClick={() => { setModalType('ingred'); setIsModalOpen(true); }}>Add Item</Button></div>
               <div className="space-y-4">
                 {inventory.ingredients.map(i => <div key={i.id} className="p-4 bg-white rounded-xl flex justify-between"><span>{i.name} ({i.brand})</span><span>{i.quantity} {i.unit}</span></div>)}
               </div>
            </div>
          )}
          {activeView === 'calendar' && (
            <div className="max-w-xl mx-auto space-y-6">
               ...
               <form onSubmit={e => { e.preventDefault(); addTask((e.target as any).task.value, (e.target as any).date.value); (e.target as any).reset(); }} className="flex gap-2">
                 <input name="task" placeholder="What to do?" className="flex-1 p-2 border rounded" />
                 <input name="date" type="date" className="p-2 border rounded" />
                 <Button type="submit">Add</Button>
               </form>
               <div className="space-y-2">
                 {tasks.map(t => <div key={t.id} className="p-4 bg-white rounded-xl flex justify-between items-center"><span>{t.title}</span><span className="text-xs text-slate-400">{t.date}</span></div>)}
               </div>
            </div>
          )}
        </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={modalType?.toUpperCase() || ''}>
         {modalType === 'setup' && (
           <form className="space-y-4" onSubmit={e => { e.preventDefault(); const f = e.currentTarget; addSetup({ name: f.sname.value, type: f.stype.value, startDate: f.sdate.value, reservoirSize: f.ssize.value, location: f.sloc.value, notes: f.snotes.value }); setIsModalOpen(false); }}>
             <input name="sname" placeholder="System Name" required className="w-full p-4 bg-slate-50 rounded-2xl border" />
             <select name="stype" className="w-full p-4 bg-slate-50 rounded-2xl border"><option>Hydroponic</option><option>Kratky</option><option>DWC</option></select>
             <input name="ssize" placeholder="Reservoir (L)" className="w-full p-4 bg-slate-50 rounded-2xl border" />
             <input name="sdate" type="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full p-4 bg-slate-50 rounded-2xl border" />
             <input name="sloc" placeholder="Location" className="w-full p-4 bg-slate-50 rounded-2xl border" />
             <textarea name="snotes" placeholder="Notes" className="w-full p-4 bg-slate-50 rounded-2xl border h-24" />
             <Button type="submit" className="w-full">Create System</Button>
           </form>
         )}
         {modalType === 'plant' && (
           <form className="space-y-4" onSubmit={e => { e.preventDefault(); const f = e.currentTarget; addPlant({ name: f.pname.value, variety: f.pvar.value, plantedDate: f.pdate.value, status: 'Healthy', notes: '' }); setIsModalOpen(false); }}>
             <input name="pname" placeholder="Species" required className="w-full p-4 bg-slate-50 rounded-2xl border" />
             <input name="pvar" placeholder="Variety" className="w-full p-4 bg-slate-50 rounded-2xl border" />
             <input name="pdate" type="date" defaultValue={new Date().toISOString().split('T')[0]} className="w-full p-4 bg-slate-50 rounded-2xl border" />
             <Button type="submit" className="w-full">Add Plant</Button>
           </form>
         )}
         {modalType === 'ingred' && (
           <form className="space-y-4" onSubmit={e => { e.preventDefault(); const f = e.currentTarget; addIngredient({ name: f.iname.value, brand: f.ibrand.value, quantity: f.iqty.value, unit: f.iunit.value }); setIsModalOpen(false); }}>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom duration-500">
      <div className="text-center space-y-4">
        ...
        <p className="text-slate-500">Describe or photograph plant symptoms for a professional diagnosis.</p>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <Card title="Describe Symptom">
          <textarea value={issue} onChange={e => setIssue(e.target.value)} placeholder="What's wrong?" className="w-full h-48 p-6 rounded-3xl border bg-slate-50 outline-none mb-6" />
          <div className="relative border-4 border-dashed rounded-3xl p-10 flex flex-col items-center justify-center mb-6 group cursor-pointer">
            {image ? (
              <div className="relative w-full h-full">
                <img src={image} className="w-full h-auto rounded-2xl" alt="Symptom" />
                <button onClick={() => setImage(null)} className="absolute -top-4 -right-4 bg-red-500 text-white p-2 rounded-full"><X size={16}/></button>
              </div>
            ) : (
              <>
                <Plus className="text-slate-300 mb-2" size={40} />
        ...
        <Card title="Diagnosis">
          {diagnosis ? (
            <div className="text-slate-700 leading-relaxed whitespace-pre-wrap p-8 bg-slate-50 rounded-3xl border">{diagnosis}</div>
          ) : (
            <div className="flex flex-col items-center justify-center h-full py-32 text-slate-300 opacity-20">
